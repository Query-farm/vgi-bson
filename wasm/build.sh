#!/usr/bin/env bash
# Build the browser (`worker:`) build of the bson VGI worker:
# bson-wasm compiled to wasm32-unknown-emscripten, linked by emcc into a
...
LIB="target/$TARGET/release/libbson_wasm.a"
...
# which the decode + Arrow encoders overflow — an overflow surfaces as a bare
# "memory access out of bounds" with no stack error. INITIAL_MEMORY must cover

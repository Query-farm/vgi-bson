...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-bson VGI
# worker, using a prebuilt standalone `haybarn-unittest` and the signed community
# `vgi` extension — no C++ build from source. Ported from vgi-units so the Docker
# image_test can drive the SAME suite over a pre-launched container.
#
...
# what LOCATION (VGI_BSON_WORKER) the .test files ATTACH:
#
#   subprocess  VGI_BSON_WORKER = the stdio worker command (DuckDB spawns it).
#               For the Docker image_test this is `docker run -i --rm … stdio`.
#   http        start `bson-worker --http` (auto port; advertises `PORT:<n>`
#               on stdout), VGI_BSON_WORKER = http://127.0.0.1:<port>.
#               If VGI_BSON_WORKER is ALREADY set to an http(s):// URL (e.g. a
#               pre-launched container in the docker image_test), it is used
#               as-is and no local worker is spawned.
#   unix        start `bson-worker --unix <sock>` (advertises `UNIX:<sock>`
#               on stdout), VGI_BSON_WORKER = unix://<sock>.
#
# Required environment:
#   HAYBARN_UNITTEST  path to the haybarn-unittest binary
# Optional:
#   WORKER_BIN        path to the compiled bson-worker binary (used to launch the
#                     http/unix servers, and the stdio LOCATION). Defaults to the
...
# release build in this repo.
WORKER_BIN="${WORKER_BIN:-$REPO/target/release/bson-worker}"
...
# Bring up the out-of-band server for http/unix and resolve VGI_BSON_WORKER.
...
start_server_and_set_location() {
  ...
  case "$kind" in
    http)
      ...
      export VGI_BSON_WORKER="http://127.0.0.1:$port"
      echo "HTTP worker ready on 127.0.0.1:$port (pid $SERVER_PID)"
      ;;
    unix)
      SOCK_PATH="${VGI_BSON_SOCK:-/tmp/bson.$$.sock}"
      ...
      export VGI_BSON_WORKER="unix://$SOCK_PATH"
...
case "$TRANSPORT" in
  subprocess)
    ...
    # VGI_BSON_WORKER (e.g. a bare `docker run … stdio` command) if the caller
    # set one.
    export VGI_BSON_WORKER="${VGI_BSON_WORKER:-$WORKER_BIN}"
    ;;
  http)
    # Honor a pre-launched HTTP worker (e.g. a running container in the docker
    # image_test): if VGI_BSON_WORKER already points at an http(s) URL, use it
    # and skip spawning a local binary. The awk preprocessor still injects
    # httpfs because TRANSPORT=http.
    if [[ "${VGI_BSON_WORKER:-}" =~ ^https?:// ]]; then
      echo "Using pre-launched HTTP worker at $VGI_BSON_WORKER"
    else
...
: "${VGI_BSON_WORKER:?worker LOCATION (stdio command, http:// URL, or unix:// socket)}"
...
# The bson suite's mongodump_read('test/data/users.bson') reads a committed
# fixture by RELATIVE path — resolved by the WORKER against ITS cwd. For local
# subprocess/http the worker's cwd is $STAGE (we `cd` there below), so stage the
# fixture too. For the Docker image_test the worker runs INSIDE the container, so
# the caller bind-mounts test/data to the container's /app/test/data instead.
if [[ -d "$REPO/test/data" ]]; then
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_BSON_WORKER, pattern: $TEST_PATTERN) ..."
...
if grep -qiE "All tests were skipped|total skipped [1-9]" "$REPORT"; then
  ...
  echo "       is NOT a pass. Transport=$TRANSPORT worker=$VGI_BSON_WORKER." >&2
  exit 1
fi
...
# warn (not fail) because it is intermittent.

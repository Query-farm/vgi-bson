#!/usr/bin/env bash
# Build the bson VGI worker and run the SQLLogic tests against it using the
...
echo "==> Building bson-worker (release)"
cargo build --release --bin bson-worker
...
WORKER="$REPO_ROOT/target/release/bson-worker"
...
VGI_BSON_WORKER="$WORKER" \
VGI_WORKER_CATALOG_NAME="bson" \

...
#
# Dispatch the single vgi-bson image into one of its transports:
...
#
# The worker is stateless (pure BLOB compute), so there is no /data to create and
# no state env to wire — each mode just exec's the binary.
set -e
...
case "${1:-http}" in
  http)
    ...
    exec bson-worker --http "$@"
    ;;
  tcp)
    shift 2>/dev/null || true
    exec bson-worker --tcp "0.0.0.0:${PORT_TCP:-8001}" "$@"
    ;;
  stdio)
    shift 2>/dev/null || true
    exec bson-worker "$@"
